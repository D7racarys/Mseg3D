...
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'MSeg3D'))
from hrnet_cfg import hrnet_w18
from fcn_cfg import fcn_head

# MSeg3D + PGCN Configuration for SemanticNuScenes
# This configuration integrates PGCN module into the MSeg3D architecture
...
model = dict(
    ...
    point_head=dict(
        type="PointSegMSeg3DPGCNHead",
        class_agnostic=False, 
        num_class=num_class,
        # PGCN configuration
        pgcn_cfg=dict(
            k=20,  # Number of neighbors for k-NN graph
            sample_rate=0.8,  # Random sampling rate for graph construction
            in_channels=32,  # Input feature dimension from voxel features
            out_channels=64,  # Output feature dimension
        ),
        model_cfg=dict(
            VOXEL_IN_DIM=32,  
            VOXEL_CLS_FC=[64],
...
data = dict(
    ...
    workers_per_gpu=4,  # Reduced workers to save memory 
    train=dict(
        type=dataset_type,
        root_path=data_root,
